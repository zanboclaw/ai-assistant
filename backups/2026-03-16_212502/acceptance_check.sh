...
wait_for_task_final() {
  ...
  while true; do
    ...
    status="$(JSON_PAYLOAD="$resp" TARGET_TASK_ID="$task_id" python3 - <<'PY'
import json
import os

raw = os.environ.get("JSON_PAYLOAD", "").strip()
task_id = os.environ.get("TARGET_TASK_ID", "").strip()

if not raw or not task_id:
    print("")
    raise SystemExit(0)
...
post_task() {
  local user_input="$1"
  USER_INPUT="$user_input" python3 - <<'PY' | curl -s -X POST "${API_BASE}/tasks" \
...
check_task_summary() {
  ...
  summary="$(JSON_PAYLOAD="$resp" TARGET_TASK_ID="$task_id" python3 - <<'PY'
import json
import os
...
try:
    ...
    print(f"ERROR: /tasks JSON 解析失败: {e}")
    raise SystemExit(1)
...
check_steps_protocol() {
  ...
  local raw
  raw="$(curl -s "${API_BASE}/tasks/${task_id}/steps")"

  JSON_PAYLOAD="$raw" python3 - "$task_id" <<'PY' > /tmp/acceptance_protocol_check.txt
import json
import os
import sys
...
assert_step_tool() {
  ...
  local actual
  actual="$(JSON_PAYLOAD="$raw" STEP_ORDER="$step_order" python3 - <<'PY'
import json
import os

raw = os.environ.get("JSON_PAYLOAD", "").strip()
step_order = int(os.environ.get("STEP_ORDER", "0"))
...
assert_step_output_contains() {
  ...
  local matched
  matched="$(JSON_PAYLOAD="$raw" STEP_ORDER="$step_order" KEYWORD="$keyword" python3 - <<'PY'
import json
import os
...
run_case_read_summarize_write() {
  ...
  local task_id
  task_id="$(echo "$post_resp" | extract_task_id)"
...
main() {
  ...
  section "API 健康检查"
  if curl -s "${API_BASE}/tasks" >/dev/null; then
