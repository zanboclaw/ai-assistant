...
SUPPORTED_OPERATORS = {"eq", "ne", "gt", "lt", "gte", "lte"}
...
def infer_structured_steps_from_user_input(user_input: str) -> list[dict]:
    ...
    if (
        ...
        and ("否则" in user_input or "不成立" in user_input or "否则写入" in user_input)
    ...
    if (
        ...
        if url_match:
            ...
            if "不等于" in user_input:
                operator = "ne"
                right_value = re.search(r"状态码\s*不等于\s*(\d+)", user_input)
            else:
                operator = "eq"
                right_value = re.search(r"状态码\s*等于\s*(\d+)", user_input)
            expected_code = int(right_value.group(1)) if right_value else 200
...
def tool_if_condition(left: Any, operator: str, right: Any) -> dict:
    try:
