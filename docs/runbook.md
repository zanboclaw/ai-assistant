...
- 另外新增了 `sandbox_file` 实验 target，可在 [apps/api/stage7_sandbox](/opt/ai-assistant/apps/api/stage7_sandbox) 下验证 file-level source-copy/apply/rollback 闭环；Web 治理台现在也内置 `sandbox_file` source-copy 模板
- `sandbox_file` payload 现在既支持直接写 `content`，也支持通过 `source_path` 从仓库源码复制到 sandbox；若省略 `content` 会做 exact copy，若同时给 `source_path + content` 则会保留 source-copy 元数据并把内容视为“基于源码副本的修改版”
- Web 治理台还补上了 workflow proposal -> `sandbox_file` source-copy bridge 模板，以及 Stage 7 Source Copy 指标卡
- 容器模式下，API 会通过只读 `WORKSPACE_ROOT=/workspace_repo` 挂载仓库源码，供 `sandbox_file` source-copy 读取真实源码文件
- `workflow-proposals/{id}/change-request-draft` 现在也可显式桥接到 `sandbox_file` target，用于验证 proposal -> file-level source-copy experiment -> rollback
- 当前白名单自动建议：
...
如果想验证 Stage 7 新增的 workflow proposal -> `sandbox_file` source-copy bridge：
...
如果想确认 Stage 7 groundwork 当前闭环已经收口，但阶段整体仍保持 `completed=false`：
...
- `bash scripts/stage7_sandbox_file_bridge_check.sh` -> Stage 7 新增 bridge 实验脚本（用于验证 workflow proposal 可显式桥接到 `sandbox_file` target，再完成 source-copy/apply -> rollback）
...
说明：以上 groundwork 项已经达到 `groundwork_completed=true`、`operational=true`；新增的 `sandbox_file` 实验通道则作为 file-level 补充能力继续推进，但 Stage 7 全量目标（sandbox/branch/code patch 自动化）仍未完成，因此整体仍保持 `completed=false`。
