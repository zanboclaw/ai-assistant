...
历史上重复度较高的增强版规划文档已移动到：

- [docs/archive/README.md](/opt/ai-assistant/docs/archive/README.md)
...
- P0 Trace 第二刀（补齐更细粒度 trace / replay 能力）
- P0 MCP Tool Registry 第二刀（补 `mcp_http` smoke / UI 编辑 / task 主链显式调用）
- P0 Minimal Skill Registry 第二刀（补治理台可视化 / skill 选择入口 / 更细粒度 skill trace）
- 通用任务架构第一期（补 `TaskIntent / DeliverableSpec / deliverable-first planner / validation`）

当前 P0 已落地的部分：

- Trace 底座第一刀
- Trace 查询接口第一刀
- Web 任务详情 `Traces` 面板第一刀
- MCP Tool Registry 第一刀
- 最小 `mcp_stdio` 注册 / 审批 / 应用 / 执行 smoke
- Minimal Skill Registry 第一刀
- 最小 skill package 导入 / discovery / 显式 skill invocation / skill trace smoke

当前已继续推进的第二刀：

- Trace 第二刀：更细粒度 trace 展示 + 只读 replay
- Minimal Skill Registry 第二刀：治理台 skill 可视化 + task UI 显式选择 skill

基于 `docs/New features needed/project_design_doc_universal_task_architecture.md` 的方向判断，下一条主线建议切到：

- Deliverable-first Universal Task Architecture
- 先补 `TaskIntent / DeliverableSpec`
- 再补 `generate + validate` 强制进入计划主链
- 再由 validation 驱动任务完成判定
...
- 任务列表与任务详情
- 步骤详情
- Trace 详情面板（Task / Step / Model / Tool）
- 待审批列表与批准/拒绝
- 风险策略查看与编辑
...
- LangGraph 决策说明：[docs/archive/langgraph_decision.md](/opt/ai-assistant/docs/archive/langgraph_decision.md)
- runtime 模块化计划：[docs/archive/runtime_boundary_plan.md](/opt/ai-assistant/docs/archive/runtime_boundary_plan.md)
- Structured step 协议草案：[docs/structured_step_protocol_v1.md](/opt/ai-assistant/docs/structured_step_protocol_v1.md)
- `json_extract` 工具设计记录：[docs/archive/next_tool_json_extract.md](/opt/ai-assistant/docs/archive/next_tool_json_extract.md)
